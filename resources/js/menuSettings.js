  ...
  async function loadMenus({ highlightIds = [] } = {}) {
    ...
    function renderMenuRows(parentId = 0, level = 0) {
      ...
      children.forEach((menu) => {
        ...
        tr.innerHTML = `
                <td class="px-4 py-2.5 text-zinc-700 dark:text-zinc-300">${indent}${menu.title}</td>
                <td class="px-4 py-2.5 text-zinc-700 dark:text-zinc-300">${targetpage || ""}</td>
                <td class="px-4 py-2.5 text-zinc-700 dark:text-zinc-300">${roles || ""}</td>
                <td class="px-4 py-2.5 text-zinc-700 dark:text-zinc-300">${parentName}</td>
